...
byte incomingSignalModes[6] = {SOURCE2SINK, SOURCE2SINK, SOURCE2SINK,
                               SOURCE2SINK, SOURCE2SINK, SOURCE2SINK};
...
void loop() {
  FOREACH_FACE(f) {
    if (!isValueReceivedOnFaceExpired(f) &&
        (int8_t)getLastValueReceivedOnFace(f) >= 0) {
      ...
      // check if neighbor has a signal type that should override ours, and is
      // trying to SEND to us
      if (incomingSignalModes[f] > signalMode &&
          incomingPropagationStates[f] == SEND) {
        ...
        if (signalMode == PUSH) {
          ...
          if (pips[f] == 0) {  // regular push
            ...
            propagationState = RESPOND;
          } else if (!isValueReceivedOnFaceExpired(
                         pushDirection)) {  // both pips and neighbor exists
    ...
    case SOURCE2SINK:
      switch (propagationState) {
        case INERT:
          if (!buttonWasPressed) {
            FOREACH_FACE(f) {
              // send color using mydata to each neighbor so that we can animate
              // chains
          ...
          break;
        case SEND:
          // As source, we need to check for which neighbor is broadcasting SEND
          // (previously RESPOND) and try to input a move on that side. If a pip
          // is already present on that face then attempt a push.
          if (isSource) {
            // If the source is clicked again before we get a response from our
            // neighbors then cancel the move. Note that we don't call
            // buttonSingleClicked() again here because the call above has
            ...
            // Is the attempted action a push because that face has a pip
            // already there?
            if (pips[neighborBroadcastingRespond] != 0) {
              // Are we trying to push a color that doesn't belong to us? If so
              // go back to idle
              if ((currentTurnColor &&
            ...
            // If the player doesn't make a move for a certain amount of time,
            // then cancel it.
            // TODO act on this timer
            // sharedTimer.set(moveCancelTimeout);
            // As sink, we need to keep broadcasting SEND for 0.5 seconds and
            // then change to INERT
          } else {
            if (!haveSetTimer) {
          ...
          // TODO: check if any neighbors are RESPOND. if they are, then trigger
          // a move. (write pip and go to INERT or, save push direction and go
          // to PUSH SEND)
          //       if we are going to PUSH SEND make sure to write to
          //       myPushColorBit
    ...
    case PUSH:
      switch (propagationState) {
        case SEND: {
          ...
          if (isPushNeighborRespond) {
            ...
            if (myData >> 1 > 0) {
              ...
              } else {  // we're only in PUSH SEND and not source if both push
                        // pips are filled
        ...
        case RESPOND: {
          ...
          FOREACH_FACE(f) {
            if (!isValueReceivedOnFaceExpired(f) &&
                (!(incomingPropagationStates[f] == RESPOND ||
                   incomingPropagationStates[f] == RESOLVE) &&
        ...
        case RESOLVE: {
          broadcastOnAllFaces();
          // we're not SEND ing so it won't matter if we send to all
          // this lets us skip keeping track of which neighbors to send RESOLVE
          // to

          bool areAllNeighborsResolveOrInert = true;
          FOREACH_FACE(f) {
            if (!isValueReceivedOnFaceExpired(f) &&
                !(incomingPropagationStates[f] == INERT ||
                  incomingPropagationStates[f] == RESOLVE)) {
    ...
    case BLOOM:
      ...
      switch (propagationState) {
        case SEND: {
          ...
          FOREACH_FACE(f) {
            if (!isValueReceivedOnFaceExpired(f) &&
                (incomingPropagationStates[f] <
                     SEND  // if it's SEND or RESOLVE we're OK
          ...
          if (!isAnyNeighborNotSendOrResolve) {
            ...
            switch (myData) {
              case UNDO:
                FOREACH_FACE(f) { pips[f] = lastPips[f]; }
                break;
              case TURN_CHANGE:
                FOREACH_FACE(f) { lastPips[f] = pips[f]; }
          ...
          // TODO: check if all neighbors are SEND. if so, switch to resolve and
          // do the action stored in myData
          break;
        }
        case RESOLVE: {
  ...
  FOREACH_FACE(f) {
    ...
    if (pips[f] == 0) {
      if (arePipsChanged) {
        brightness = map(sin8_C(map(millis() % PENDING_CHANGE_PULSE_WIDTH, 0,
                                    PENDING_CHANGE_PULSE_WIDTH, 0, 255)),
                         0, 255, 0, 100);
      }
      if (!currentTurnColor) {
      ...
      if (pips[f] == incomingNeighborData[f] ||
          (pips[f] == 1 && redCount >= 2) || (pips[f] == 2 && blueCount >= 2)) {
        brightness = map(sin8_C(map(millis() % CHAIN_PW, 0, CHAIN_PW, 0, 255)),
                         0, 255, 200, 255);
      }
      if (pips[f] == 1) {
        setColorOnFace(dim(RED, brightness), f);
